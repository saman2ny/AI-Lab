...
export async function explainValues(values) {
    ...
    const message = await client.messages.create({
        ...
        messages: [
            {
                role: "user",
                content: JSON.stringify(values.map((v) => ({ key: v.key, name: v.name, value: v.value, unit: v.unit, status: v.status, ref: v.ref }))),
            },
        ],
        tools: [EXPLAIN_TOOL],
...
function genericExplanation(v) {
    const verdict = v.status === "normal"
