...
import { explainValues } from "../services/ai/explainValuesPrompt.js";
import { prioritizeWalkthroughValues } from "../services/ai/walkthroughPriority.js";
import { setReport, getReport } from "../services/sessionStore/inMemoryStore.js";
...
async function handleUpload(req, res, files) {
    const { text, warnings } = await normalizeUpload(files.map((f) => ({ originalname: f.originalname, mimetype: f.mimetype, buffer: f.buffer })));
    ...
    const classified = extraction.values.map((raw) => {
    ...
    const explanations = await explainValues(classified.map((v) => ({ key: v.key, name: v.name, value: v.value, unit: v.unit, status: v.status, ref: v.ref })));
...
reportRouter.get("/pdf", requireAuth, async (req, res) => {
    ...
    if (!report)
        return res.status(404).json({ message: "No report in this session." });
    try {
