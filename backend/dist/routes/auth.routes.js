...
const cookieOpts = { httpOnly: true, sameSite: "lax", secure: config.nodeEnv === "production" };
...
authRouter.post("/login", asyncHandler(async (req, res) => {
    const { email, password } = req.body;
    if (!email || !password)
        return res.status(400).json({ ok: false, message: "Enter your email and password." });
...
authRouter.post("/google", asyncHandler(async (req, res) => {
    ...
    if (!idToken)
        return res.status(400).json({ ok: false, message: "Missing Google token." });
...
authRouter.post("/verify", asyncHandler(async (req, res) => {
    const userIdFromCookie = req.cookies?.[PENDING_COOKIE];
    const { userId: requestedUserId, code } = req.body;
    const userId = userIdFromCookie ?? requestedUserId;
    if (!userId || !code)
        return res.status(400).json({ ok: false, message: "Start sign-in again." });
...
authRouter.post("/verify/resend", asyncHandler(async (req, res) => {
    const userId = req.cookies?.[PENDING_COOKIE];
    if (!userId)
